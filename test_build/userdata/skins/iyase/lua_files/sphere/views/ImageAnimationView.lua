	...
	if not range then
		...
		for i = range[1], range[2], range[1] < range[2] and 1 or -1 do
			...
			if file then
				...
				images[i] = gfx_util.newPixel(0, 0, 0, 0)
			end
		end
	end
	self.images = images
...
function ImageAnimationView:draw()
	...
	if self.quad then
		love.graphics.draw(self.image, self.quads[animation.frame], self.x, self.y, 0, sx, sy, ox, oy)
		return
	end
	love.graphics.draw(self.images[animation.frame], self.x, self.y, 0, sx, sy, ox, oy)
end
